...
class Ferrscrape:
    ...
    def parse(self, response):
        ...
        for elem in part_elems:
            ...
            detail_part_elems = description_elem.css('a.part-name')
            for detail_part_elem in detail_part_elems:
                ...
                data['description'] = description_elem.text(strip=True)
                data['detail'] = detail_part_elem.text(strip=True)
                try:
                    data['key'] = elem.css_first('button.btn.btn-success.btn-icon.btn-sm').text(strip=True)
                except:
                    data['key'] = ''
                data['price'] = elem.css_first('div.d-block').text(strip=True)
                datas.append(data)
                # print(data)
